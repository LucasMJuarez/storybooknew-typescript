...
const SecondaryText = ({title, description, btnText, onClick: handleClick, tabIndexButton, role, ariaLabel}: Props) => {
    let {wrapperCard, card, container, wrapperText, wrapperImg, button} = styles;
    return (
        <div className={wrapperCard}>
            <div className={card} role={role} aria-label={ariaLabel}>
                <div className={wrapperImg}>
                    <MovistarImg />
                </div>
                <div className={container}>
                    <div className={wrapperText}>
                        <Title level={1} align='Left' type='text_primary_special_1' text={title} />
                        <Sub level={1} align='Left' type='text_primary_inverse_2' text={description} />
                    </div>

                    <div className={button}>
                        <Small text={btnText} inverse={true} onClick={() => handleClick()} tabIndex={tabIndexButton} />
                    </div>
                </div>
            </div>
        </div>
